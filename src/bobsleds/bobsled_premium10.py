...
# Lightning arcs and bolt particles
arc_timer = 0
bolt_particles = []

def add_bolt_particles(x, y):
    for _ in range(5):
        bolt_particles.append([x, y, random.randint(3, 6), random.randint(50, 100), BOLT_BLUE])

def update_bolt_particles():
    global arc_timer
    arc_timer += 1
...
def draw_bobsled(pos_x, pos_y, angle=0):
    ...
    # Add bolts if flashing
    if arc_timer % 12 < 6:
        add_bolt_particles(pos_x, pos_y)
